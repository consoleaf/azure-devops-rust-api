...
pub mod endpointproxy {
  ...
  pub mod execute_service_endpoint_request {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::ServiceEndpointRequestResult> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::ServiceEndpointRequestResult =
  ...
  pub mod get_service_endpoints {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::ServiceEndpointList> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::ServiceEndpointList = serde_json::from_slice(&bytes).map_err(|e| {
          azure_core::error::Error::full(
            azure_core::error::ErrorKind::DataConversion,
            e,
            format!(
  ...
  pub mod create {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::ServiceEndpoint> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::ServiceEndpoint = serde_json::from_slice(&bytes).map_err(|e| {
          azure_core::error::Error::full(
            azure_core::error::ErrorKind::DataConversion,
            e,
            format!(
  ...
  pub mod list {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::ServiceEndpointTypeList> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::ServiceEndpointTypeList =
          serde_json::from_slice(&bytes).map_err(|e| {
            azure_core::error::Error::full(
              azure_core::error::ErrorKind::DataConversion,
              e,
    ...
    impl std::future::IntoFuture for RequestBuilder {
      ...
      type IntoFuture =
        futures::future::BoxFuture<'static, azure_core::Result<models::ServiceEndpointTypeList>>;
      #[doc = "Returns a future that sends the request and returns the parsed response body."]
      #[doc = ""]
      #[doc = "You should not normally call this method directly, simply invoke `.await` which implicitly calls `IntoFuture::into_future`."]
      #[doc = ""]
  ...
  pub mod list {
    ...
    impl Response {
      pub async fn into_body(
        ...
        let bytes = self.0.into_body().collect().await?;
        let body: models::ServiceEndpointExecutionRecordList = serde_json::from_slice(&bytes)
          .map_err(|e| {
            azure_core::error::Error::full(
              azure_core::error::ErrorKind::DataConversion,
              e,
