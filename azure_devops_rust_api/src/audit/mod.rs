  ...
  pub mod list {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::AuditActionInfoList> {
        ...
        let body: models::AuditActionInfoList = serde_json::from_slice(&bytes).map_err(|e| {
  ...
  pub mod query {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::AuditLogQueryResult> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::AuditLogQueryResult = serde_json::from_slice(&bytes).map_err(|e| {
          azure_core::error::Error::full(
            azure_core::error::ErrorKind::DataConversion,
            e,
            format!(
        ...
        Box::pin({
          ...
          async move {
            ...
            if let Some(start_time) = &this.start_time {
              let formatted_date_time = crate::date_time::format_date_time(start_time)?;
              req
                .url_mut()
                .query_pairs_mut()
  ...
  pub mod query_all_streams {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::AuditStreamList> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::AuditStreamList = serde_json::from_slice(&bytes).map_err(|e| {
          azure_core::error::Error::full(
            azure_core::error::ErrorKind::DataConversion,
            e,
            format!(
