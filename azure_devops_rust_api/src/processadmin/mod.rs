  ...
  pub mod list {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::AdminBehaviorList> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::AdminBehaviorList = serde_json::from_slice(&bytes).map_err(|e| {
          azure_core::error::Error::full(
            azure_core::error::ErrorKind::DataConversion,
            e,
            format!(
  ...
  pub mod import_process_template {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::ProcessImportResult> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::ProcessImportResult = serde_json::from_slice(&bytes).map_err(|e| {
          azure_core::error::Error::full(
            azure_core::error::ErrorKind::DataConversion,
            e,
            format!(
  ...
  pub mod import_process_template_status {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::ProcessPromoteStatus> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::ProcessPromoteStatus = serde_json::from_slice(&bytes).map_err(|e| {
          azure_core::error::Error::full(
            azure_core::error::ErrorKind::DataConversion,
            e,
            format!(
