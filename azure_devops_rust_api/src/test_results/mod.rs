...
pub mod testlogstoreendpoint {
  ...
  pub mod get_test_log_store_endpoint_details_for_sub_result_log {
    ...
    pub struct Response(azure_core::Response);
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::TestLogStoreEndpointDetails> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::TestLogStoreEndpointDetails =
  ...
  pub mod query {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::TestResultMetaDataList> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::TestResultMetaDataList = serde_json::from_slice(&bytes).map_err(|e| {
          azure_core::error::Error::full(
            azure_core::error::ErrorKind::DataConversion,
            e,
            format!(
    ...
    impl std::future::IntoFuture for RequestBuilder {
      ...
      type IntoFuture =
        futures::future::BoxFuture<'static, azure_core::Result<models::TestResultMetaDataList>>;
      #[doc = "Returns a future that sends the request and returns the parsed response body."]
      #[doc = ""]
      #[doc = "You should not normally call this method directly, simply invoke `.await` which implicitly calls `IntoFuture::into_future`."]
      #[doc = ""]
  ...
  pub mod update {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::TestResultMetaData> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::TestResultMetaData = serde_json::from_slice(&bytes).map_err(|e| {
          azure_core::error::Error::full(
            azure_core::error::ErrorKind::DataConversion,
            e,
            format!(
