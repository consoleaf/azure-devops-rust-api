  ...
  pub mod list {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::InstalledExtensionList> {
        ...
        let body: models::InstalledExtensionList = serde_json::from_slice(&bytes).map_err(|e| {
    ...
    impl RequestBuilder {
      ...
      pub fn include_disabled_extensions(mut self, include_disabled_extensions: bool) -> Self {
      ...
      pub fn include_installation_issues(mut self, include_installation_issues: bool) -> Self {
    ...
    impl std::future::IntoFuture for RequestBuilder {
      ...
      type IntoFuture =
        futures::future::BoxFuture<'static, azure_core::Result<models::InstalledExtensionList>>;
  ...
  pub mod update {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::InstalledExtension> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::InstalledExtension = serde_json::from_slice(&bytes).map_err(|e| {
          azure_core::error::Error::full(
            azure_core::error::ErrorKind::DataConversion,
            e,
            format!(
