  ...
  pub mod list {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::PermissionsReportList> {
        ...
        let body: models::PermissionsReportList = serde_json::from_slice(&bytes).map_err(|e| {
    ...
    impl std::future::IntoFuture for RequestBuilder {
      ...
      type IntoFuture =
        futures::future::BoxFuture<'static, azure_core::Result<models::PermissionsReportList>>;
  ...
  pub mod get {
    ...
    impl Response {
      pub async fn into_body(self) -> azure_core::Result<models::PermissionsReport> {
        let bytes = self.0.into_body().collect().await?;
        let body: models::PermissionsReport = serde_json::from_slice(&bytes).map_err(|e| {
          azure_core::error::Error::full(
            azure_core::error::ErrorKind::DataConversion,
            e,
            format!(
