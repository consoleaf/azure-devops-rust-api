...
impl Patcher {
  ...
  fn patch_operation_status_in_releases(
    ...
    match key {
      ...
      ["definitions", "DeploymentQueryParameters", "properties", "operationStatus"] => Some(
        self.patched_operation_status("Query deployment based on deployment operation status."),
      ),
  ...
  fn patch_probation_retries_type(&mut self, key: &[&str], value: &JsonValue) -> Option<JsonValue> {
    // Only applies to hooks specs
    if !self.spec_path.ends_with("serviceHooks.json") {
      return None;
    }
  ...
  fn patch_docs(&mut self, key: &[&str], value: &JsonValue) -> Option<JsonValue> {
    match key {
      ...
      [.., "description" | "summary"] => value.as_str().map(|v| self.patch_docstring(v).into()),
      _ => None,
    }
  }
  ...
  fn patch_wit_create_update_item(&mut self, key: &[&str], value: &JsonValue) -> Option<JsonValue> {
        ...
        for param in value.members_mut() {
          if let Some(s) = param["name"].as_str() {
            ...
            if s == "body" {
              ...
              param["description"] =
                JsonValue::from("A list of operations to perform when creating a Work Item");
            }
          }
        }
        Some(value)
        ...
        for param in value.members_mut() {
          if let Some(s) = param["name"].as_str() {
            ...
            if s == "body" {
              ...
              // Update the description to reflect the definition
              param["description"] =
                JsonValue::from("A list of operations to perform when updating a Work Item");
            }
          }
        }
        Some(value)
  ...
  fn patch_identity_descriptors(&mut self, _key: &[&str], value: &JsonValue) -> Option<JsonValue> {
    if let JsonValue::Object(obj) = value {
      if let Some("#/definitions/IdentityDescriptor") = obj.get("$ref").and_then(|s| s.as_str()) {
        let mut value = value.clone();
        value.remove("$ref");
        if value
